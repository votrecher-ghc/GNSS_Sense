...
%     try
%         % 步骤2.1: 获取该时刻的接收机位置和钟差 (这是所有计算的基础)
...
%         clock_corrected_pr = P_raw - c * (receiver_clk_err - sat_state.clock_error);
...
%             initial_phase_anchor_cycles = (P_raw - c * (receiver_clk_err - sat_state.clock_error)) / lambda_carrier;
%             accumulated_residual_phase = 0;
%         end
...
%     catch ME
%         % 可以在此添加错误处理，例如: fprintf('处理历元 %d 时出错: %s\n', epoch_idx, ME.message);
%     end
% end
% fprintf('--> 核心数据处理循环完成！\n\n');
% % --- 3. 数据修剪与后处理 ---
...
%     % datetick('x', 'HH:MM:ss', 'keepticks', 'keeplimits');
% else
%     amplitude_norm_smooth = amplitude_norm; % 如果数据不足，则不进行平滑
%     fprintf('    数据点不足，跳过幅度平滑处理。\n');
% end
% % ========================= (旧)新增代码结束 =========================
% % --- 绘图模块 3: 去趋势后的伪距 ---
% % 作用: 显示移除轨道趋势后的伪距残差，对应论文图2(b)。
% % 如何禁用: 在下面 "figure" 到 "datetick" 的所有行前加上 '%' 符号。
% % -------------------------------------------------------------------------
% fprintf('--> 正在处理和绘制: 去趋势后的伪距...\n');
% window_size_pr = 25; % 可根据采样率调整
% pr_trend = movmean(processed_pr_vector, window_size_pr, 'omitnan');
% pseudorange_detrend = processed_pr_vector - pr_trend;
% % figure('Name', sprintf('卫星 %s - 去趋势后的伪距', target_satellite_id), 'NumberTitle', 'off');
% % plot(time_vector_cst, pseudorange_detrend, 'b-');
% % title(sprintf('卫星 %s - 去趋势后的伪距 (滑动平均法)', target_satellite_id));
% % xlabel('时间 (北京时间)');
% % ylabel('残余伪距 (米)');
% % grid on;
% % datetick('x', 'HH:MM:ss', 'keepticks', 'keeplimits');
% % --- 绘图模块 4: 相位去趋势 (方法A - 滑动平均法) ---
% % 作用: 使用滑动平均法移除相位主趋势，简单直观，对应论文图2(c)的其中一种实现。
% % 如何禁用: 在下面 "figure" 到 "datetick" 的所有行前加上 '%' 符号。
% % -------------------------------------------------------------------------
% fprintf('--> 正在处理和绘制: 相位去趋势 (滑动平均法)...\n');
% window_size_phase = 25; % 可根据采样率调整
...
% % figure('Name', sprintf('卫星 %s - 去趋势相位 (滑动平均)', target_satellite_id), 'NumberTitle', 'off');
% % plot(time_vector_cst, phase_detrend_movmean, 'r-');
% % title(sprintf('卫星 %s - 去趋势GPSense相位 (滑动平均法)', target_satellite_id));
% % xlabel('时间 (北京时间)');
% % ylabel('去趋势相位 (弧度)');
% % grid on;
% % datetick('x', 'HH:MM:ss', 'keepticks', 'keeplimits'); 
...
% % ========================= 新增三合一绘图模块开始 =========================
% fprintf('--> 正在生成三合一对比图...\n');
% figure('Name', sprintf('卫星 %s - 三合一分析图', target_satellite_id), 'NumberTitle', 'off', 'Position', [100, 100, 800, 900]);
% 
% % 子图 1: 平滑处理后的归一化幅度
% subplot(3, 1, 1);
% plot(time_vector_cst, amplitude_norm_smooth);
...
% % if length(time_vector) > 10 % 确保有足够的数据点进行滤波
% %     % 精确计算采样率
% %     mean_dt = mean(seconds(diff(time_vector)));
% %     Fs = 1 / mean_dt;
% %     fprintf('    计算出的平均采样率为: %.2f Hz\n', Fs);
% %     
% %     % 设计并应用带通滤波器 (0.2Hz - 5.0Hz)
% %     f_low = 0.2;  % 低截止频率
% %     f_high = 3; % 高截止频率
% %     order = 4;    % 滤波器阶数
% %     [b, a] = butter(order, [f_low f_high] / (Fs / 2), 'bandpass');
% %     phase_detrend_bpf = filtfilt(b, a, unwrapped_phase); % 使用零相位滤波器避免延迟
% %     
...
% %     datetick('x', 'HH:MM:ss', 'keepticks', 'keeplimits');
% % else
% %     phase_detrend_bpf = NaN(size(time_vector)); % 如果数据不够，返回NaN
% %     fprintf('    数据点不足，跳过带通滤波处理。\n');
% % end
% fprintf('\n✅ 所有分析和绘图任务完成！\n');
% end
