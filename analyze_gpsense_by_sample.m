% ============== analyze_gpsense_by_sample.m (按采样点绘图版 v1.1 - 新增SNR三合一图) ==============
%
% 功能:
%   本脚本是按采样点数进行GPSense分析和绘图的版本。所有功能与
%   analyze_gpsense_ultimate.m 完全相同，但横坐标使用采样点数而非时间。
%   所有绘图均使用默认颜色。
%   【V1.1更新】: 新增包含原始SNR的三合一绘图模块。
%
% 输出:
...
%
function [time_vector, amplitude_norm, pseudorange_detrend, phase_detrend_movmean, phase_detrend_bpf] = analyze_gpsense_by_sample(obs_data, nav_data, target_satellite_id, epoch_range)
% --- 1. 参数检查与初始化 ---
...
for loop_idx = 1:num_epochs_to_process
    ...
    try
        ...
        raw_snr_vector(valid_results_count) = cn0_dbhz;
...
raw_snr_vector = raw_snr_vector(1:valid_results_count);
% --- 3.5 核心计算分离 (确保变量总是被定义) ---
sample_points = 1:valid_results_count; % <--- 新增: 创建采样点坐标轴
...
% plot(sample_points, amplitude_vector);
% title(sprintf('卫星 %s - 原始幅度 (未归一化)', target_satellite_id));
% xlabel('采样点数 (Sample Number)');
% ylabel('原始幅度 (sqrt(W))');
% grid on;
% --- 绘图模块 1.5: 原始SNR观测值 ---
...
% plot(sample_points, raw_snr_vector, '.-');
% ylim([30 50]);
...
% plot(sample_points, amplitude_norm);
...
% plot(sample_points, amplitude_norm_smooth);
...
% plot(sample_points, processed_pr_vector);
...
% plot(sample_points, processed_pr_vector, 'DisplayName', '修正后、未去趋势伪距');
% hold on;
% plot(sample_points, pseudorange_detrend, 'LineWidth', 1.5, 'DisplayName', '最终去趋势伪距');
% hold off;
...
% % 子图 2: 去趋势后的伪距
% subplot(3, 1, 2);
% plot(sample_points, pseudorange_detrend);
...
% % 子图 3: 去趋势相位 (滑动平均法)
% subplot(3, 1, 3);
% plot(sample_points, phase_detrend_movmean);
...
%     plot(sample_points, phase_detrend_bpf);
...
figure('Name', sprintf('卫星 %s - 三合一分析图 (SNR/幅度/相位)', target_satellite_id), 'NumberTitle', 'off', 'Position', [200, 80, 800, 900]); % 调整位置避免重叠
...
subplot(3, 1, 2);
plot(sample_points, amplitude_norm_smooth);
title('平滑后的归一化幅度 (Savitzky-Golay)');
xlabel('采样点数 (Sample Number)');
ylabel('归一化幅度');
grid on;
