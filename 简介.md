...
最后更新：2026-08-12 17:05:57 +08:00
...
- `runtime_benchmark/`：独立的在线认证流程与运行开销测试，覆盖认证时延、CPU 时间、内存、吞吐量、冷启动和输入规模变化。
- `runtime_benchmark/results/latest/`：最近一次正式开销测试的逐次样本、汇总指标、规模变化结果与论文表述草稿。
- `gesture_analysis/scientific_graphing/PAPER_FIGURE_STYLE.md`：论文实验图的字体、线宽、画布和导出规范。
- `gesture_analysis/scientific_graphing/reflow_trajectory_gallery.py`：将轨迹重建画廊整理为紧凑 6×2 布局的工具。
- `gesture_analysis/scientific_graphing/standardize_environment_figure_set.py`：统一环境对比四个子图画布、绘图区和文字层级的工具。
- `gesture_analysis/scientific_graphing/harmonize_evaluation_pdf_fonts.py`：按论文实际插入宽度协调实验图视觉字号的工具。
- `gesture_analysis/scientific_graphing/compact_attack_defense_rates.py`：生成宽扁、完整网格版 Attack-defense rates 矢量图的工具。
- `physical_consistency/`：2026-05 之后新增的物理/时空一致性模块，负责 NAV-only 天空图、卫星几何距离、NAV-OBS 距离残差、时空唯一性和时空认证通过率实验。
- `physical_consistency/spatiotemporal_consistency_section.md`：时空一致性章节中文草稿，可直接给论文相关小节继续改写。
...
- `physical_consistency/outputs/spatiotemporal_uniqueness/`：时空唯一性、距离差分布、时空认证通过率实验图和 CSV 输出目录。
- `physical_consistency/outputs/skyplot_time_comparison/`：同一地点不同时间天空图对比图和 CSV 输出目录。
- `修改记录.md`：2026-05 之后的逐次修改记录。后续每次实际改文件后都要同步追加记录。
- `CHANGELOG.md`：2026-08 之后与论文同步工作的精简变更记录。

截至 2026-08-12，给下一个 agent 的最新交接重点：

1. 主论文工作流仍然是 `gesture_analysis` 下的 `Data-Driven trajectory recovery + Score_k authentication + paper figure export`。
2. 新增研究线是 `physical_consistency`：认证不只要求恢复轨迹像模板，还要求候选时间/地点满足距离域时空一致性。
3. 时空一致性当前不并入 `Score_k` 分数，而是作为认证阶段的额外条件：`GestureTemplatePass AND SpatiotemporalConsistencyPass`。
4. 目前放弃了“卫星扰动簇一致性”作为主判据，因为几十公里/几十分钟尺度下天空图分布可能仍然相似；当前主判据改为多卫星距离残差 `r_obs_i - rho_GT_i`。
5. 在线认证流程已从绘图与离线评估逻辑中独立出来；正式开销测试结果为平均时延 100.26 ms、P95 104.39 ms、P99 106.75 ms、平均 CPU 时间 97.03 ms、峰值内存 109.52 MiB、吞吐量 9.97 次/秒。
6. Fig. 10 轨迹重建画廊已由 4×3 改为 6×2，论文中的纵向占用约减半，实验轨迹和数值未改变。
7. Fig. 17 四个环境对比子图已统一为 `800×800 pt` 矢量画布和相同绘图区比例；混淆矩阵标签、图例、色条与视觉字号已协调。
8. Fig. 16 Attack-defense rates 已改为约 `1056×575 pt` 的宽扁构图，顶部单行图例，并使用横纵双向完整网格。
9. 实验结果图的字体不再机械使用同一源字号，而是以 Fig. 11(b) 在论文中的视觉大小为参考，根据实际插入宽度逐图协调。
...
- 结果图优先导出为矢量 PDF；论文中的最终视觉字号由“源字号 × 实际插入比例”共同决定。
- 同组子图应统一源画布、绘图区比例、边距和 LaTeX 插入宽度，不能只统一 `includegraphics` 的宽或高。
- 需要保持指定主图尺寸一致，不要因为局部修改破坏 PNG/PDF 版式。
- 每次修改图形脚本后，原则上完整重新导出一版图。

### 0.1 2026-08 新增能力与验证结果

#### 在线认证与开销测试

`runtime_benchmark/stardial_pipeline.py` 实现了一条仅包含在线认证所需阶段的流程：C/N0 预处理、衍射特征提取、LoS 几何反演、12 类手势模板匹配、物理一致性验证和联合判决。绘图、文件输出、RINEX 解析与离线阈值标定不计入稳态认证时延。

`runtime_benchmark/benchmark_runtime.py` 负责预热、重复计时、CPU 时间、内存、吞吐量、冷启动和规模变化测试；`runtime_benchmark/test_pipeline.py` 用于基本功能回归。最近一次正式结果保存在 `runtime_benchmark/results/latest/`，论文采用的核心指标如下：

| 指标 | 结果 |
|---|---:|
| 平均认证时延 | 100.26 ms |
| P95 / P99 时延 | 104.39 / 106.75 ms |
| 平均 CPU 时间 | 97.03 ms |
| CPU 占用 | 单个逻辑核的 96.78% |
| 峰值内存 | 109.52 MiB |
| 顺序吞吐量 | 9.97 次/秒 |
...
- Fig. 10：`reflow_trajectory_gallery.py` 将原 4×3 轨迹画廊整理为 6×2，保留原轨迹数据并减少纵向占用。
- Fig. 17：`standardize_environment_figure_set.py` 与 `square_environment_metrics.py` 统一四个环境对比子图；前者负责整组画布与文字层级，后者可单独生成认证指标子图。
- 实验图字体：`harmonize_evaluation_pdf_fonts.py` 按论文中的实际显示尺寸协调高度敏感性、距离差、时空通过率、t-SNE/PCA 等图的视觉字号。
- Fig. 16：`compact_attack_defense_rates.py` 生成宽扁构图，并在横、纵两个方向显示完整网格。

上述脚本只负责认证开销测量或论文图排版复现，不应改变实验数据和认证判据。
...
flowchart TD
    ...
    A --> P["Physical Consistency<br/>NAV-only range / OBS residual / spacetime check"]
    P --> E
    A --> F["科研绘图层"]
    B --> F
    C --> F
...
### 4.8 Physical Consistency：距离域时空一致性模块

职责：

- 使用 RINEX NAV、OBS、候选时间和候选经纬高，构建认证阶段的物理一致性证据；
- 计算给定地点/时间下的 NAV-only 理论卫星天空图和几何距离；
- 将 OBS raw pseudorange 结合卫星钟差和接收机钟差校正为观测距离；
- 按同一时间轴和同一卫星 ID 对齐，计算 `r_obs_i - rho_GT_i`；
- 用多卫星残差 RMS 或分布变化判断候选时间/地点是否与授权时空点一致。

当前认证语义：

```text
通过认证 = 手势模板相似度通过 AND 时空一致性通过
```

注意：

- `physical_consistency` 目前是独立研究模块，还没有被硬接入 `gesture_analysis/workflow/layer4_authenticate_gestures.m`。
- 它不应该修改 `Score_k` 的定义；`Score_k` 仍只衡量恢复轨迹像不像模板。
- 时空一致性用于认证阶段的额外约束，重点证明“不同地点同一时间”和“同一地点不同时间”可被区分。
- 标准 GNSS 天空图使用当地 ENU 坐标系，以真北/真东/天顶为参考，不随接收机机身 yaw 旋转；若后续需要机身坐标天空图，需要额外引入姿态参数。
...
├─ data/                              原始数据
├─ physical_consistency/              NAV-only 天空图、距离残差和时空一致性实验
...
`physical_consistency/` 也需要单独理解：它不是 `gesture_test.m` 的一部分，而是 2026-05 之后新增的认证物理约束实验模块。它主要服务论文中“时空唯一性/时空一致性认证”的新增论证，核心输入是 NAV、OBS、候选时间和候选经纬高，核心输出是卫星天空图、卫星几何距离、NAV-OBS 距离残差、空间/时间认证通过率以及相关论文图。
...
这些脚本位于 `physical_consistency/`，通常单独运行，不需要先跑 `gesture_test.m`：

- `example_skyplot_from_nav.m`  
  输入 NAV、时间、经纬高，绘制该时刻该地点的 NAV-only 天空图；如果提供 OBS，还会根据 OBS 估计接收机位置并绘制 OBS 天空图做对比。

- `calculate_and_plot_skyplot_from_nav.m`  
  底层函数：从 NAV 计算卫星方位角、仰角、几何距离和可见性。

- `calculate_satellite_ranges_from_nav.m`  
  底层函数：给定 NAV、时间、经纬高，纯计算每颗卫星到该点的几何距离 `rho_GT_i`。

- `test_nav_range_time_series.m`  
  测试脚本：画给定时间后一段时间内每颗卫星几何距离随时间变化，支持把大基线距离剥离，只看小量级波动。

- `calculate_nav_obs_range_residuals.m`  
  核心函数：对齐 NAV 理论距离和 OBS 校正伪距，输出 `residual_obs_minus_gt_m = r_obs_i - rho_GT_i`。

- `example_nav_obs_range_residuals.m`  
  示例脚本：选一组 NAV/OBS 和一个固定点，计算并绘制多系统距离残差。

- `example_spatiotemporal_uniqueness_figures.m`  
  论文图脚本：生成空间唯一性热力图/剖面图、时间唯一性曲线、卫星距离差分布图、分位数带图、小提琴 envelope 和 fan chart。

- `example_spatiotemporal_auth_pass_rate.m`  
  论文图脚本：生成“不同地点同一时间认证通过率”和“同一地点不同时间认证通过率”。当前默认 `reuse_existing_csv = true`，只改图样式时会直接读已有 CSV。

- `example_skyplot_time_comparison.m`  
  论文/分析脚本：同一地点、不同时间天空图对比。当前默认比较 `2 h`，每 `15 min` 采样一次，输出天空图轨迹、共同可见卫星数量和角位移统计。
...
- `scenario_authentication_roc.pdf` 已与 `scenario_authentication_metrics_bar.pdf` 统一为固定 `1200 x 900 pt` 页面。
- 修复位置在 `gesture_analysis/scientific_graphing/export_paper_figures_data_driven.m` 的固定 PDF 画布导出逻辑。
...
- `scenario_authentication_metrics_bar.pdf` 已与 `scenario_authentication_roc.pdf` 统一为固定 `1200 x 900 pt` 页面。
- 如果后续重新导出完整论文图，这两个 scenario PDF 不应再被 `exportgraphics` 自动裁成不同页面尺寸。
...
### 9.x 新增：`physical_consistency/outputs/spatiotemporal_uniqueness/`

这是 2026-05 之后新增的时空一致性实验输出目录，包含 `pdf/`、`png/`、`fig/`、`csv/` 四类结果。

核心图：

- `figure_space_uniqueness_heatmap`  
  同一时间、不同空间位置的距离残差 RMS 热力图，用于证明空间唯一性。

- `figure_space_uniqueness_cross_section`  
  空间偏移剖面图，用于直观看东西/南北方向位置偏移如何导致距离残差变大。

- `figure_time_uniqueness_full_range`  
  同一地点、不同时间偏移下的距离残差 RMS 全范围曲线。

- `figure_time_uniqueness_near_time_zoom`  
  近授权时间局部放大图。灰色授权点虚线已按用户要求去除，避免视觉上产生 10 m 与 0 m 的误解。

- `figure_satellite_range_difference_distribution`  
  卫星距离差分位数带图。浅蓝阴影表示 `5%-95% quantile band`，深蓝阴影表示 `25%-75% quantile band`，深色线表示 `Median (Q50)`。

- `figure_satellite_range_difference_violin_envelope`  
  在 `100/200/300/400/500 m` 空间偏移处画离散小提琴图，展示每个偏移下不同卫星距离差的分布形状。

- `figure_satellite_range_difference_fan_chart`  
  fan chart 版本，叠加理论 `+d/-d` 边界，用于说明候选点空间偏移为 `d` 时单颗卫星几何距离差的理论范围。

- `figure_spatial_auth_pass_rate`  
  同一时间、不同地点的认证通过率图。当前标注 `tau_ST = 11.02 m`，本实验中 `d <= 10 m` 仍有非零通过样本，`d >= 15 m` 后通过率为 `0%`。

- `figure_temporal_auth_pass_rate`  
  同一地点、不同时间的认证通过率图。当前标注 `tau_ST = 11.02 m`，本实验中只有 `Delta t = 0 s` 存在非零通过样本，`|Delta t| >= 0.1 s` 后通过率为 `0%`。

关键 CSV：

- `spatiotemporal_auth_threshold_calibration.csv`  
  保存时空一致性阈值标定结果。当前 `tau_ST` 来自授权时空点 epoch 级 RMS 的 `95%` 分位数，约为 `11.016 m`。

- `spatial_auth_pass_rate.csv` / `temporal_auth_pass_rate.csv`  
  保存空间/时间认证通过率曲线数据。

- `satellite_range_difference_distribution.csv` / `satellite_range_difference_distribution_summary.csv`  
  保存空间偏移下每颗卫星的几何距离差样本和分位数统计。

### 9.x 新增：`physical_consistency/outputs/skyplot_time_comparison/`

同一地点不同时间天空图对比输出目录。

当前默认结果：

- 起始时间：`2026-01-08 05:59:39.116 UTC`
- 时间窗：`2 h`
- 采样间隔：`15 min`
- 位置：`35.77204196 deg, 120.02477210 deg, 92.889 m`

核心文件：

- `pdf/figure_skyplot_time_comparison_2h.pdf`
- `png/figure_skyplot_time_comparison_2h.png`
- `fig/figure_skyplot_time_comparison_2h.fig`
- `csv/skyplot_time_comparison_2h_all_epochs.csv`
- `csv/skyplot_time_comparison_2h_endpoint_delta.csv`
- `csv/skyplot_time_comparison_2h_summary.csv`
...
- 2 小时后可见卫星 `57` 颗；
- 共同可见卫星 `42` 颗；
- 共同可见卫星天空角位移中位数约 `43.10 deg`，最大约 `71.90 deg`。

注意：脚本默认 `show_satellite_labels = false`，因为 60 颗左右卫星全标名称会很拥挤。若需要单颗卫星 ID，请优先看 `endpoint_delta.csv`。
...
- `physical_consistency/outputs/spatiotemporal_uniqueness/`

主要保存：

- `pdf/`：时空唯一性、距离差分布、认证通过率论文图；
- `png/`：对应预览图；
- `fig/`：MATLAB 可编辑图；
- `csv/`：阈值标定、空间/时间通过率、距离差分布等原始统计表。

- `physical_consistency/outputs/skyplot_time_comparison/`

主要保存：

- 同一地点不同时间天空图对比图；
- 每个采样 epoch 的 NAV-only 天空图表；
- 起点/终点共同卫星的方位角、高度角和天空角位移统计。

一般来说：

- 想复查时空唯一性的图，看 `spatiotemporal_uniqueness/pdf/`；
- 想复查通过率阈值，看 `spatiotemporal_auth_threshold_calibration.csv`；
- 想复查 2 小时天空图差异，看 `skyplot_time_comparison_2h_summary.csv` 和 `skyplot_time_comparison_2h_endpoint_delta.csv`。
...
6. `physical_consistency/` 当前是认证物理约束实验模块，不属于 `gesture_test.m` 默认主流程；后续若要真正接入认证系统，应新增接口，而不是直接改写 `Score_k`。
...
1. 先确认 NAV/OBS 文件路径和目标时间是否匹配；
2. 再确认候选经纬高是否是授权点或故意偏移点；
3. 如果只是改图样式，优先使用已有 CSV，不要重新跑耗时残差计算；
4. 如果修改 `example_spatiotemporal_auth_pass_rate.m`，注意 `reuse_existing_csv` 开关；
5. 如果修改天空图，注意标准 GNSS 天空图是 ENU 坐标系，不是接收机机身坐标系。
